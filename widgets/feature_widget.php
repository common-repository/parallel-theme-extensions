 ...
 */
class Parallel_Feature_Widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array( 'classname' => 'wcp_image', 'description' => __('Add a feature to the homepage features section.', 'parallel') );
        parent::__construct( 'Parallel_feature', __('Parallel - Feature Widget', 'parallel'), $widget_ops );
...
?>
	<p>
        <label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Feature Title', 'parallel'); ?></label>
        <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($instance['title']); ?>" />
    </p>
    <p>
        <label for="<?php echo $this->get_field_id('text'); ?>"><?php esc_html_e('Feature Icon Class', 'parallel'); ?></label>
        ...
        <?php _e('Copy and paste the required icon prefix and class from the', 'parallel'); ?> <a href="<?php echo esc_url('https://fontawesome.com/icons?d=gallery&m=free'); ?>" target="_blank"><?php _e('FontAwesome Icons list', 'parallel'); ?></a> <?php _e('and choose from 1500+ icons. For example, to use the smiley face icon enter', 'parallel'); ?> <code><?php _e('far fa-smile', 'parallel'); ?></code>
    </p>
    <p>
        <label for="<?php echo $this->get_field_id('image_url'); ?>"><?php esc_html_e('Feature Image', 'parallel'); ?></label>
    ...
    <p>
        <label for="<?php echo $this->get_field_id('textarea'); ?>"><?php esc_html_e('Feature Description', 'parallel'); ?></label>
...
add_action( 'widgets_init', function() {
    register_widget( 'Parallel_Feature_Widget' );
} );
?>